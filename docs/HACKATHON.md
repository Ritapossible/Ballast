...
| `bitget-mcp-server` (US stock data) | ✅ **integrated as a second opinion on the calendar** — `ballast/mcp.py` speaks MCP 2024-11-05 over HTTP to `agent.bitget.com/mcp` with no dependencies, and `ballast/crosscheck.py` asks its earnings-calendar entry the same question Nasdaq was asked for every decision on the chain, publishing where the two agree. **The entry id is resolved from the live catalog rather than pinned**, because pinning it failed: the upstream renamed `equity_calendar_earnings` to `equity_calendar`, the old id answered `Unknown entry_id`, and all 120 rows were recorded `unknown` for a day. That is the fail-closed rule working — never agreement — but the page went on quoting the last good number, so the row is now rendered from the file rather than typed. A second fault sat behind it: the give-up rule stopped the run after three consecutive failures even once the service had answered, so an intermittent 404 abandoned 109 rows. Down and flaky are different faults and only the first is a reason to stop early. Current run: **116 of 120 decisions checked, 4 recorded `unknown`** — **114 agree, 2 disagree**. The four are ADBE and ORCL on 2026-09-09 and 09-10, *the same two hedges this project already discloses as defect 3f* (the selector matched a session without checking the release time and covered a night early). A second source, asked independently, landed on exactly the rows already marked wrong. Re-verified live 2026-09-23: `equity_price_quote` and `equity_calendar_earnings` both return 200. It deliberately does **not** decide: re-pointing the selector days before a submission would invalidate the record the change is meant to support. An unreachable service is still recorded as `unknown` with the upstream's own status code, never as agreement — a second source that fails open is worse than none. |
