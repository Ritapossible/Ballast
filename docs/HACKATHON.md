...
| Agent Hub CLI + `--paper-trading` | ⚠️ **live and exercised nightly; the exchange refuses the order** — credentials are configured, `bgc` installs and answers, and every session from 2026-09-14 records `venue: bgc-paper · Bitget Agent Hub, paper-trading`. Each of the five hedges since then was routed to the Agent Hub and came back `HTTP 400: exchange environment is incorrect`. Ballast then simulated the fill and wrote the exchange's own error onto the row as `venue_fallback`, which is the designed behaviour and is now proven in production rather than only in tests. **No fill on the ledger carries an exchange `orderId`.** **The cause is measured, not guessed: Bitget's demo environment does not list a single tokenized stock perpetual.** Across all three demo product types it carries nine contracts — `SBTCSUSDT`, `SETHSUSDT`, `SXRPSUSDT`, `SBTCSUSD`, `SETHSUSD`, `SBTCSUSDU26`, `SETHSUSDU26`, `SBTCSPERP`, `SETHSPERP` — every one of them BTC, ETH or XRP. The live `USDT-FUTURES` list carries 797 including all twelve names in this book. So no position Ballast holds can be filled on demo under any credentials, which is what the exchange means by *environment is incorrect*. Checked 2026-09-19, in one command: `curl -s "https://api.bitget.com/api/v2/mix/market/contracts?productType=SUSDT-FUTURES"`. |
...
| `bitget-signal` research Skills | ⚠️ **integrated; the service is returning no data** — `mcp.signal()` reaches all 19 tools at `datahub.noxiaohao.com/mcp` over the same transport, no account or API key. Every one answers with an empty envelope: `{"error": ""}`, `{"alt_me_error": ""}`, or 44 news feeds each carrying `items: []`. Confirmed to be the service rather than this client by calling the same server through a second, unrelated MCP client on a different network and getting the identical empty result. `signal_headlines()` flattens the per-feed envelopes so a dead upstream reads as **no articles** rather than 44 headlines that are really source names. Checked 2026-09-19. |
| `bitget-mcp-server` (US stock data) | ✅ **integrated as a second opinion on the calendar** — `ballast/mcp.py` speaks MCP 2024-11-05 over HTTP to `agent.bitget.com/mcp` with no dependencies, and `ballast/crosscheck.py` asks its `equity_calendar_earnings` entry the same question Nasdaq was asked for every decision on the chain, publishing where the two agree. It deliberately does **not** decide: re-pointing the selector days before a submission would invalidate the record the change is meant to support. An unreachable service is recorded as `unknown` with the upstream's own status code, never as agreement — a second source that fails open is worse than none. |
| Playbook / GetAgent Studio | ✅ **package authored and validated** — `playbook/ballast-overnight-protection/` is a `trade_strategy` Playbook over 10 Bitget RWA stock perpetuals, each confirmed `symbolStatus: normal` / `isRwa: YES` against the public contract config and probed for replayable 1h bars before `backtest_support: full` was declared. It passes the platform's own `scripts/validate.py`. It publishes the **protection leg in isolation** and says so: not an alpha strategy, no directional claim, read the return as the price of removing those nights. The 10 protected nights are the real earnings dates in the replay window, from the same Nasdaq calendar the live selector uses. `.github/workflows/playbook.yml` uploads and backtests it with `ACCESS-KEY` from the `PLAYBOOK_API_KEY` secret, so the key never reaches a transcript or an argv. |
