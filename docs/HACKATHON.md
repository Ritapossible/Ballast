...
| `bitget-mcp-server` (US stock data) | ❌ **not used** — the earnings calendar comes from Nasdaq directly and prices from Bitget's public API. A second source for the calendar would be a genuine improvement and is on the roadmap, not in this submission. |
