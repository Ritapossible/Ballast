...
| Agentic Account + `--paper-trading` | ⚠️ **live and exercised nightly; the exchange refuses the order** — credentials are configured, `bgc` installs and answers, and every session from 2026-09-14 records `venue: bgc-paper · Bitget Agent Hub, paper-trading`. Each of the five hedges since then was routed to the Agent Hub and came back `HTTP 400: exchange environment is incorrect`. Ballast then simulated the fill and wrote the exchange's own error onto the row as `venue_fallback`, which is the designed behaviour and is now proven in production rather than only in tests. **No fill on the ledger carries an exchange `orderId`.** The most likely cause is that the configured key is a live-account key: Bitget Demo Trading needs a separate Demo API key, created from Personal Center → API Key Management while the account is in Demo Trading mode, and a live key used with `--paper-trading` is rejected for exactly this reason. |
