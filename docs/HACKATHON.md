...
| Playbook / GetAgent Studio | ❌ **not used** — the handbook points these at Alpha Factory for backtesting. Ballast's research suite and replay live in `research/`, reproducible with no key. Migrating the loop onto Studio now would break a signed hash chain for a checkbox. |
