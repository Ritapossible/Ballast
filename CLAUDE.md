...
36. **The track is scored 50% on paper-trading Sharpe, max drawdown and win rate.**
    `ballast/metrics.py` computes them on the live log, for the book and for the same book
    unhedged. Declining to *claim* Sharpe is not a reason to withhold it. Lead with
    drawdown - that is what insurance is measured in - and label the Sharpe as noise while
    n is small.
37. **Anything generated from `facts.json` must be regenerated by the nightly.** The README
    was checked by CI but rebuilt by nobody, so a re-measurement turned CI red.

38. **A counterfactual is derived by calling the rule, never by reimplementing it.**
    `night.calendar_call` runs the same `policy.decide` the nightly run runs with the
    model's judgment removed, so the reader-vs-calendar page cannot drift from the rule it
    claims to stand in for. It returns `action: None` if the volatility gate is ever
    re-enabled, because the rule is then no longer a function of the calendar alone and a
    NO_HEDGE would be a guess wearing the shape of an answer. Every `decided_by == "rule"`
    row is a live test of the derivation - on those nights the recorded decision *is* the
    calendar's - and the match count is published beside the claim, not asserted in a
    docstring.

39. **A re-derivation over a public feed must be pinned, and must be the fallback.**
    Nasdaq populates its release-time flag for upcoming dates and drops it to
    `time-not-supplied` for past ones, so re-deriving a past night is strictly coarser
    than what the live run saw. So the decision record now carries the rule's own answer
    at decision time, `ballast/counterfactual.py` prefers it, and a derived row is pinned
    on first build - a rebuild must never silently restate a figure an earlier build
    published because a feed moved underneath it.

## 7b. Operations

- `.github/workflows/nightly.yml` — decides at 21:00 UTC, settles at 14:30 UTC, Mon–Fri.
  Verifies the ledger chain, refreshes the coverage index and the reader-vs-calendar
  comparison, rebuilds the pages, commits `state/` and `docs/`. Crons are set
