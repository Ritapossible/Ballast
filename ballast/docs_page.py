...
and cumulative returns preserved. Studio then showed <strong>Paper Degraded &middot;
pending_validation</strong>, and <strong>no Studio decision has been recorded</strong> - so
its 0.00% is an empty account, not a night this system declined. Two things follow, and
both are stated here rather than left to be discovered: the $10,000 paper balance is not a
Track 2 run log, and <strong>the signed hash-chained ledger is</strong>. The
<code>/my-playbooks</code> API does not see Studio paper tracking at all - it lists
follow-trade deployments - so that state is only readable in the Studio UI.</p>
