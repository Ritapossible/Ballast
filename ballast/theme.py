...
/* ---- chrome ---- */
.top{{position:sticky;top:0;z-index:30;background:rgba(7,7,8,.9);
...
.nav{{position:sticky;top:57px;z-index:29;background:rgba(7,7,8,.9);
  backdrop-filter:blur(14px);border-bottom:1px solid var(--line);
...
.bul li{{display:flex;gap:14px;padding:8px 0;color:var(--mid);font-size:15.5px}}
.bul li::before{{content:"";width:6px;height:6px;border-radius:50%;
  background:var(--accent);flex:none;margin-top:11px}}
...
@media(max-width:640px){{section{{padding:62px 0}} .nav{{top:55px}}}}
